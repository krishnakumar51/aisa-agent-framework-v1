...
class TestingDatabaseManager:
    ...
    async def create_task(self, instruction: str, platform: str = "auto-detect", 
        ...
        base_path = f"generated_code"  # Base will be extended with seq_id
    ...
    async def create_workflow_steps(self, seq_id: int, agent_name: str, steps: List[Dict]) -> List[int]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            for i, step_data in enumerate(steps):
                cursor = await db.execute("""
                    INSERT INTO workflow_steps (seq_id, agent_name, step_name, step_order, action_type, expected_result)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    seq_id, agent_name, 
    ...
    async def create_agent_communication(self, seq_id: int, from_agent: str, to_agent: str,
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                INSERT INTO agent_communications (seq_id, from_agent, to_agent, message_type, message_content)
                VALUES (?, ?, ?, ?, ?)
    ...
    async def save_generated_file(self, seq_id: int, agent_name: str, file_name: str, 
                                 file_path: str, file_type: str, version: int = 1) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                INSERT INTO generated_files (seq_id, agent_name, file_name, file_path, file_type, version, file_size)
                VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    async def create_testing_environment(self, seq_id: int, environment_type: str, venv_path: str) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                INSERT INTO testing_environments (seq_id, environment_type, venv_path)
                VALUES (?, ?, ?)
    ...
    async def update_testing_environment(self, seq_id: int, **kwargs):
        ...
        for key, value in kwargs.items():
            if key in ['requirements_installed', 'appium_server_running', 'playwright_installed', 
    ...
    async def save_test_execution(self, seq_id: int, script_version: int, execution_attempt: int,
                                 success: bool, **kwargs) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                ...
                (seq_id, script_version, execution_attempt, success, execution_output, 
                 error_details, screenshot_before, screenshot_after, ocr_before, ocr_after, execution_duration)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
    ...
    async def get_task_info(self, seq_id: int) -> Optional[Dict]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                SELECT * FROM automation_tasks WHERE seq_id = ?
            """, (seq_id,))
    ...
    async def export_to_csv(self, seq_id: int, export_path: str):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """, (seq_id,))
            communications = await cursor.fetchall()
        ...
        with open(export_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow([task_info['seq_id'], task_info['instruction'], 
            ...
            for step in workflow_steps:
                writer.writerow([step['step_id'], step['step_name'], step['action_type'], 
            ...
            for exec_row in executions:
                writer.writerow([exec_row[2], exec_row[3], exec_row[4], exec_row[9], exec_row[10]])
